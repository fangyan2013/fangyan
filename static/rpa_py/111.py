...
def run(playwright: Playwright) -> None:
    ...
    page.fill("[placeholder=\"请输入标题\"]", "111")

    # Click text=请选择 无匹配数据 加载中 >> span
    page.click("text=请选择 无匹配数据 加载中 >> span")

    # Click form li:has-text("执行>Test>3.185")
    page.click("form li:has-text(\"执行>Test>3.185\")")

    # Click button:has-text("确认")
    # with page.expect_navigation(url="http://192.168.3.185/dpa/workflow/edit?modelId=mdf7da25&execId=ee1607331978343&modelName=111&changeProcess=false&execIp=192.168.3.185&createUser=ui170800&instanceId="):
    with page.expect_navigation():
    ...
    # Click text=100%断开连线删除 >> :nth-match(div, 4)
    page.click("text=100%断开连线删除 >> :nth-match(div, 4)")
